    ...
    def __stem(self):
        ...
        for index, documents in enumerate(self.documents):
            for headers in self.documents[index].keys():
                try:
                    self.documents[index][headers][0] = [self.lemmatizer.lemmatize(tokens) for tokens in lists for lists in self.documents[index][headers][0]]
                except:
                    pass
...
if __name__ == '__main__':
    main(file_path=rf'./processed/lowercasing/lpdf.json',output_file=rf'./processed/stemmed/lemmatized-pdf.json')
